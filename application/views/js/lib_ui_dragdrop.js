 ...
 * lib_ui_dragdrop.js
 *
 * A helper class to simplify dragging and dropping
 * This Javascript file contains the standard javascript for the entire framework, included in the header
 * 
 * This class supports placeholders which are drop targets, placeholders are replaced on dropping by the dragging elements
 * 
 * 
 * IMPORTANT NOTE:
 * --> To define conditions to when a drop is allowed, inherit this class and override: isDropAllowed()
 * --> To add/replace dragging behavior, inherit: onDragStart(), onDragEnd(), onDragOver() and onDrop()
 * 
 * This class contains of draggables and droppables:
 * -> The draggables are the items you want to drag
 * -> The droppables are the drop containers where you can drop your draggables
 * 
 * TO START:
 * 1. add draggables with addDraggable() 
 * 2. add droppables with addDroppable()
 * 3. remove draggables with removeDraggable()
 * 4. remove droppables with removeDroppable()
 * 5. write conditions for drops by overriding isDropAllowed() when necessary
 * 6. to extend functionality of dragging and dropping override and inherit parent behavior of onDragStart(), onDragEnd(), onDragOver() and onDrop()
 * 
 *************************************************************************
 * WARNING:
 *************************************************************************
 ...
 * 
 * 28 dec 2024 lib_global_ui_dragdrop.js created* 
 */
?>
class UIDragDrop
{
    ...
    {        
        ...
        {            
            ...
            // console.log("start drag " + arrElements[iIndex]); 
        }
    ...
    {        
        ...
        {
            //we come here when it's a draggable (no margin) or the droppable parent is the same (then we have margin)
            //look for nearest element to drop before or after
            ...
            // console.log("nearest element: 123123123123123");
            // console.log(objNearestElement);
            // console.log("droppable: 123123123123123");
            // console.log(objDroppable);
            // console.log("Parent arrDragSources[0].parentElement:");
            // console.log(arrDragSources[0].parentElement);
            // console.log("objEvent.target.parentElement:");
            // console.log(arrDragSources[0].parentElement);
    ...
    handleDrop(objEvent, objDroppable)
    {
        ...
        {
            // const objNearestElement = this.getNearestElement(arrSiblingDraggables, objEvent.clientX, objEvent.clientY);
            ...
            for (let iIndex = 0; iIndex < iLenDragSources; iIndex++)
            {
                ...
                if (this.objDraggingNearestElement)
                {
                    if (this.bDraggingInsertBeforeOrAfter)
                    {
                        this.objDraggingNearestElement.before(arrDragSources[iIndex]);
                    }
                    else
                    {
                        this.objDraggingNearestElement.after(arrDragSources[iIndex]);
                    }
                }    
        ...
        console.log("DragDrop class: drop PARENT ===============");
     ...
     * by default this function selects all HTML elements in the document that have CSS class this.sCSSClassSelected
     * 
    ...
    {
        return [...document.getElementsByClassName(this.sCSSClassSelected)];
    }
    ...
    removeCSSClassAcceptDraggingFromDroppables()
    {
        ...
        {    
            this.arrDroppables[iIndex].classList.remove(this.sCSSClassAcceptDragging);    
        }
    }
