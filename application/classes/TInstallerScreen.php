...
abstract class TInstallerScreen
{
	...
	protected $sConfigPathFramework;
	...
	{	
		global $sCMSRootPath;
		$this->sConfigPathFramework = $sCMSRootPath.DIRECTORY_SEPARATOR.'configs'.DIRECTORY_SEPARATOR.'application_'.$_SERVER['SERVER_NAME'].'.php';
	...
	private function handleInstallerEnabled()
	{
		...
		{
			$objConfig = new TConfigFileFramework();
			if (file_exists($this->sConfigPathFramework))
			{
	 ...
	 * 
	 * WARNING: a loaded framework config file is assumed!
	 *
	 * @return bool true=ok (when no check and password is ok), false=password not ok
	 */
	private function checkInstallPassword()
	{
		...
		// 		//save
		// 		$objConfig->saveFile($this->sConfigPathFramework);
		// 	}
		// }
		$objConfig = new TConfigFileFramework();
		if (file_exists($this->sConfigPathFramework))//load existing config file (so we can overwrite values)
	...
	/**
	 * disable installer in framework config file
	 */
	protected function disableInstaller()
	{
		...
		if (file_exists($this->sConfigPathFramework))//LOAD: existing config file (so we can overwrite values)
		{
			if (!$objConfig->loadFile($this->sConfigPathFramework))
			{
				error_log(__FILE__.': disableInstaller loading config file failed: '.$this->sConfigPathFramework);
			}
			else
			{
				...
				if (!$objConfig->saveFile($this->sConfigPathFramework))
					error_log(__FILE__.': disableInstaller saving config file failed: '.$this->sConfigPathFramework);
			}	
		}
	}	
