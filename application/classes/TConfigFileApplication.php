...
class TConfigFileApplication extends TConfigFilePHPConstants
{
	...
	protected function defineConstants()
	{
		return array(
			...
			array('APP_URL_ADMIN', '', TP_STRING, 'Local path of the CMS without trailing slash (/). This is stored in the framework config file, because it is the same for all connected websites'),			
			...
			array('APP_APPLICATIONNAME', 'Archimedes CMS', TP_STRING, 'Name of application that the CMS respresents'),			
			...
			array('APP_ADMIN_ENABLESIGNINWITHGOOGLE', false, TP_BOOL, 'Allow Google login for CMS'),			
			array('APP_ADMIN_SHOWWEBSITESINNAVIGATION', true, TP_BOOL, 'enable or disable the visibility websites in header and menu on left. This disabled makes it a webapp instead of a CMS'),			
			array('APP_ADMIN_SAVESUCCESSNOTIFICATION', true, TP_BOOL, 'show notification when record saved successfully'),			
			array('APP_ADMIN_FIRSTPAGECONTROLLER', 'loginform', TP_STRING, 'The name of the controller that represents the first page of the CMS. When you type in the URL it needs to go to the first page of the CMS.'),			
			array('APP_ADMIN_RATELIMITER_ENABLE', true, TP_BOOL, 'CMS rate limiter: enabled or not'),			
			array('APP_ADMIN_RATELIMITER_REQUESTS', 28800, TP_INTEGER, 'CMS rate limiter: [X] requests allowed per [Y] seconds. This is the X (Y: see setting below). Default: 1 request per second for 8 hours per ip address: 60*60*8=28800'),			
			array('APP_ADMIN_RATELIMITER_PERSECS', 86400, TP_INTEGER, 'CMS rate limiter: [X] requests allowed per [Y] seconds. This is the Y (X: see setting above). Default: 8 hours usage per day'),			
			...
			array('APP_ADMIN_THEME', 'default', TP_STRING, 'The directory name of the CMS theme in themes directory. THIS IS CASE SENSIVE ALSO ON WINDOWS!!! DRIVE LETTERS ARE UPPERCASE!'),
