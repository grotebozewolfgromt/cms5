...
class TConfigFileApplication extends TConfigFilePHPConstants
{
	...
	protected function defineConstants()
	{
		return array(
			...
			array('APP_DEMOMODE', false, TP_BOOL, 'The demo mode restricts access to certain parts of the Admin panel and websites to show potential users its functionality. The cronjob will set and reset sample data. To enable demo mode: set value to true and run cronjob'),
			...
			array('APP_PATH_DOMAIN_ADMIN', 'localhost', TP_STRING, 'Almost the same as APP_URL_ADMIN, but it only contains the domain name. This is separate from APP_PATH_WWW because sometimes you only need the domain and its faster to define it instead of figuring it out wasting CPU cycles'),
			array('APP_URL_ADMIN', '', TP_STRING, 'Local path of the Admin panel without trailing slash (/). This is stored in the framework config file, because it is the same for all connected websites'),			
			array('APP_PATH_UPLOADS_PUBLIC', '', TP_STRING, 'Local path of all user uploads without trailing slash (/) or backslash (\). This is the centralized place where all the user uploads are stored for all websites like a CDN (because we can only upload files to ONE server for multiple sites). THIS IS CASE SENSIVE ALSO ON WINDOWS!!! DRIVE LETTERS ARE UPPERCASE!'),			
			array('APP_URL_UPLOADS_PUBLIC', '', TP_STRING, 'URL where all uploads are stored without trailing slash (/). This is the centralized place where all the user uploads are stored for all websites like a CDN (because we can only upload files to ONE server for multiple sites)'),			
			array('APP_PATH_UPLOADS_PRIVATE', '', TP_STRING, 'Local path unreachable by browser where uploads are stored that are sensitive in nature. Path without trailing slash (/) or backslash (\). THIS IS CASE SENSIVE ALSO ON WINDOWS!!! DRIVE LETTERS ARE UPPERCASE!'),			
			array('APP_ADMIN_MODULESDIR', 'modules', TP_STRING, 'Name of the modules directory'),			
			array('APP_APPLICATIONNAME', 'Archimedes CMS', TP_STRING, 'Name of application that the Admin panel respresents'),			
			array('APP_VERSION', '5.0.0.0', TP_STRING, 'Version number of the framework'),			
			array('APP_ADMIN_ANYONECANREGISTERACCOUNT', false, TP_BOOL, 'this option SUPERSEDES the option in the settings screen. It can be switched off here for security reasons: When you would get into the database you could check this and do EVERYTHING with the whole system (because you can add a user).'),			
			array('APP_ADMIN_ENABLESIGNINWITHGOOGLE', false, TP_BOOL, 'Allow Google login for Admin panel'),			
			array('APP_ADMIN_SHOWWEBSITESINNAVIGATION', true, TP_BOOL, 'enable or disable the visibility websites in header and menu on left. This disabled makes it a webapp instead of a Admin panel'),			
			array('APP_ADMIN_SAVESUCCESSNOTIFICATION', true, TP_BOOL, 'show notification when record saved successfully'),			
			array('APP_ADMIN_FIRSTPAGECONTROLLER', 'loginform', TP_STRING, 'The name of the controller that represents the first page of the Admin panel. When you type in the URL it needs to go to the first page of the Admin panel.'),			
			array('APP_ADMIN_RATELIMITER_ENABLE', true, TP_BOOL, 'Admin panel rate limiter: enabled or not'),			
			array('APP_ADMIN_RATELIMITER_REQUESTS', 28800, TP_INTEGER, 'Admin panel rate limiter: [X] requests allowed per [Y] seconds. This is the X (Y: see setting below). Default: 1 request per second for 8 hours per ip address: 60*60*8=28800'),			
			array('APP_ADMIN_RATELIMITER_PERSECS', 86400, TP_INTEGER, 'Admin panel rate limiter: [X] requests allowed per [Y] seconds. This is the Y (X: see setting above). Default: 8 hours usage per day'),			
			array('APP_ADMIN_LOGINONLYWHITELISTEDIPS', true, TP_BOOL, 'Login only allowed by whitelisted IP addresses. Default is true because it is the most secure'),			
			array('APP_ADMIN_STEALTHMODEBLACKWHITELISTEDIPS', true, TP_BOOL, 'In stealth mode: all blacklisted and non-whitelisted IP addresses are served 404 errors instead of 401 errors. This prevents automated directory enumeration. Default is true because it is the most secure, but you might think that your website is down when you try to login from a non-whitelisted or blacklisted IP address'),
			array('APP_ADMIN_THEME', 'default', TP_STRING, 'The directory name of the Admin panel theme in themes directory. THIS IS CASE SENSIVE ALSO ON WINDOWS!!! DRIVE LETTERS ARE UPPERCASE!'),
			...
			array('APP_ADMIN_PRINTING_ZPL_ENABLE', false, TP_BOOL, 'Enable printing for zebra network printers'),			
			array('APP_ADMIN_PRINTING_ZPL_PRINTER1_NAME', 'First printer', TP_STRING, 'The name of the first printer so the user can recognize it'),			
			array('APP_ADMIN_PRINTING_ZPL_PRINTER1_IPADDRESS', 'localhost', TP_STRING, 'The IP address of the first Zebra network printer'),			
			array('APP_ADMIN_PRINTING_ESCPOS_ENABLE', false, TP_BOOL, 'Enable printing for ESC/POS network printers'),			
			array('APP_ADMIN_PRINTING_ESCPOS_PRINTER1_NAME', 'First printer', TP_STRING, 'The name of the first printer so the user can recognize it'),			
			array('APP_ADMIN_PRINTING_ESCPOS_PRINTER1_IPADDRESS', 'localhost', TP_STRING, 'The IP address of the first ESC/POS network printer'),			
			array('APP_ADMIN_PRINTING_ESCPOS_PRINTER1_PORT', 9100, TP_INTEGER, 'The tcp/ip port of the first ESC/POS network printer'),			
			array('APP_ADMIN_IMAGE_RESIZE_MAX_WIDTHPX', 3840, TP_INTEGER, 'Resolution (width) for automatic resizing an image to maximum-size format'),			
			array('APP_ADMIN_IMAGE_RESIZE_MAX_HEIGHTPX', 3840, TP_INTEGER, 'Resolution (height) for automatic resizing an image to maximum-size format'),			
			array('APP_ADMIN_IMAGE_RESIZE_LARGE_WIDTHPX', 1024, TP_INTEGER, 'Resolution (width) for automatic resizing an image to large-size format'),			
			array('APP_ADMIN_IMAGE_RESIZE_LARGE_HEIGHTPX', 1024, TP_INTEGER, 'Resolution (height) for automatic resizing an image to large-size format'),			
			array('APP_ADMIN_IMAGE_RESIZE_MEDIUM_WIDTHPX', 500, TP_INTEGER, 'Resolution (width) for automatic resizing an image to medium-size format'),			
			array('APP_ADMIN_IMAGE_RESIZE_MEDIUM_HEIGHTPX', 500, TP_INTEGER, 'Resolution (height) for automatic resizing an image to medium-size format'),			
			array('APP_ADMIN_IMAGE_RESIZE_THUMBNAIL_WIDTHPX', 150, TP_INTEGER, 'Resolution (width) for automatic resizing an image to thumbnail format'),			
			array('APP_ADMIN_IMAGE_RESIZE_THUMBNAIL_HEIGHTPX', 150, TP_INTEGER, 'Resolution (height) for automatic resizing an image to thumbnail format'),			
